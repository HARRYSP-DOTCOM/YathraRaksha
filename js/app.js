...
const App = {
  ...
  registerEventListeners() {
    ...
    document.getElementById("alert-filters")?.addEventListener("click", (e) => {
      const pill = e.target.closest(".alert-filter-pill");
      if (!pill) return;
      document.querySelectorAll(".alert-filter-pill").forEach(p => p.classList.remove("active"));
      pill.classList.add("active");
      this.renderAlerts(pill.getAttribute("data-filter"));
    });
  ...
  /**
   * Render risk alerts from local RoadDatabase
   */
  renderAlerts(filter = "all") {
    const container = document.getElementById("alerts-list");
    const budgetSummary = document.getElementById("alerts-budget-summary");
    if (!container) return;

    const roads = window.RoadDatabase?.roads || [];
    if (roads.length === 0) {
      container.innerHTML = `<div class="glass-card text-center" style="padding: 40px 20px;">
        <div style="font-size: 36px; margin-bottom: 12px;">📡</div>
        <h3>No road data available</h3>
        <p class="text-muted" style="font-size: 13px; margin-top: 6px;">Road database is loading…</p>
      </div>`;
      return;
    }

    // Derive alerts from road data
    const alerts = roads.map(road => {
      const rec = road.accidentRecords || {};
      const rating = rec.safetyRating || 10;
      let severity, icon;
      ...
      const overrunPct = budgetOverrun
        ? (((road.spentBudget - road.sanctionedBudget) / road.sanctionedBudget) * 100).toFixed(1)
        : 0;

      let description = rec.primaryCauses || "No specific issues recorded.";
      if (budgetOverrun) {
        description += ` Budget overrun of ${overrunPct}% detected — potential contractor overcharging.`;
      }
      ...
      return {
        ...
        budgetOverrun,
        overrunPct,
        accidents: rec.annualAccidents || 0,
        fatalities: rec.fatalities || 0,
        potholesPerKm: rec.potholesPerKm || 0,
        safetyRating: rating
      };
    });
    ...
    if (filtered.length === 0) {
      container.innerHTML = `<div class="glass-card text-center" style="padding: 40px 20px;">
        ...
        <p class="text-muted" style="font-size: 13px; margin-top: 6px;">No road corridors match the selected severity filter.</p>
      </div>`;
    } else {
        ...
        <div class="alert-card severity-${alert.severity}">
          ...
          <div class="alert-content">
            <div class="alert-header">
              <h4>${this.escapeHTML(alert.road.name)}</h4>
              <span class="alert-severity-badge ${alert.severity}">${alert.severity}</span>
              ${alert.budgetOverrun ? '<span class="alert-severity-badge critical">💸 Budget Overrun</span>' : ''}
            </div>
            <p class="alert-desc">${this.escapeHTML(alert.description)}</p>
            <div class="alert-stats">
              <span class="alert-stat">🚨 <strong>${alert.accidents}</strong> accidents/yr</span>
              <span class="alert-stat">💀 <strong>${alert.fatalities}</strong> fatalities</span>
              <span class="alert-stat">🕳️ <strong>${alert.potholesPerKm}</strong> potholes/km</span>
              <span class="alert-stat">⭐ Safety: <strong>${alert.safetyRating}/10</strong></span>
            </div>
          </div>
          <div class="alert-actions">
            <button onclick="window.App.focusRoadOnMap('${alert.road.id}')" class="btn btn-secondary btn-sm">Map</button>
            <button onclick="window.App.selectRoadForReport('${alert.road.id}')" class="btn btn-primary btn-sm">Report</button>
          </div>
        </div>
      `).join("");
    }
    ...
    // Budget summary
    if (budgetSummary) {
      let totalSanctioned = 0, totalSpent = 0, overrunCount = 0;
      roads.forEach(r => {
        totalSanctioned += r.sanctionedBudget;
        totalSpent += r.spentBudget;
        if (r.spentBudget > r.sanctionedBudget) overrunCount++;
      });
      const totalOverrun = totalSpent - totalSanctioned;
      const isOverrun = totalOverrun > 0;

      budgetSummary.innerHTML = `
        <div class="budget-summary-grid">
          <div class="budget-summary-item">
            <span class="budget-label">Total Sanctioned</span>
            <span class="budget-value">₹${(totalSanctioned / 10000000).toFixed(1)} Cr</span>
          </div>
          <div class="budget-summary-item ${isOverrun ? 'overrun' : 'healthy'}">
            <span class="budget-label">Total Spent</span>
            <span class="budget-value">₹${(totalSpent / 10000000).toFixed(1)} Cr</span>
          </div>
          <div class="budget-summary-item ${isOverrun ? 'overrun' : 'healthy'}">
            <span class="budget-label">${isOverrun ? 'Total Overrun' : 'Under Budget'}</span>
            <span class="budget-value">${isOverrun ? '⚠️' : '✅'} ₹${(Math.abs(totalOverrun) / 10000000).toFixed(1)} Cr</span>
          </div>
          <div class="budget-summary-item">
            <span class="budget-label">Flagged Projects</span>
            <span class="budget-value">${overrunCount} / ${roads.length}</span>
          </div>
        </div>
      `;
    }
  ...
  /**
   * Render safety scoreboard from local RoadDatabase
   */
  renderScoreboard() {
    const roads = window.RoadDatabase?.roads || [];
    ...
    // Summary metrics
    const totalAccidents = roads.reduce((sum, r) => sum + (r.accidentRecords?.annualAccidents || 0), 0);
    const avgSafety = (roads.reduce((sum, r) => sum + (r.accidentRecords?.safetyRating || 0), 0) / roads.length).toFixed(1);
    const overrunCount = roads.filter(r => r.spentBudget > r.sanctionedBudget).length;

    // Best contractor
    const sorted = [...roads].sort((a, b) => b.contractorPerformance - a.contractorPerformance);
    const best = sorted[0];

    const set = (id, val) => { const el = document.getElementById(id); if (el) el.textContent = val; };
    set("score-avg-safety", `${avgSafety} / 10`);
    set("score-total-accidents", totalAccidents.toLocaleString());
    set("score-overrun-count", `${overrunCount} of ${roads.length}`);
    set("score-best-contractor", `★${best.contractorPerformance}`);

    // Road safety rankings (sorted by safety rating descending)
    const rankingsEl = document.getElementById("score-road-rankings");
    if (rankingsEl) {
      const ranked = [...roads]
        .filter(r => r.accidentRecords)
        .sort((a, b) => (b.accidentRecords.safetyRating || 0) - (a.accidentRecords.safetyRating || 0));

      rankingsEl.innerHTML = ranked.map((road, idx) => {
        const rec = road.accidentRecords;
        const rating = rec.safetyRating || 0;
        const rankClass = idx < 3 ? `rank-${idx + 1}` : "rank-other";
        const scoreClass = rating >= 8 ? "safety-score-good" : rating >= 5 ? "safety-score-warning" : "safety-score-critical";
        const statusBadge = rating >= 8
          ? '<span class="badge badge-good">Healthy</span>'
          : rating >= 5
            ? '<span class="badge badge-sh">Warning</span>'
            : '<span class="badge badge-warn">Critical</span>';

        return `<tr>
          <td><span class="rank-badge ${rankClass}">${idx + 1}</span></td>
          <td><strong>${this.escapeHTML(road.id)}</strong><br><small style="color:var(--text-muted);">${this.escapeHTML(road.name)}</small></td>
          <td class="${scoreClass}">${rating} / 10</td>
          <td>${rec.annualAccidents}</td>
          <td>${rec.fatalities}</td>
          <td>${rec.potholesPerKm}</td>
          <td>${statusBadge}</td>
        </tr>`;
      }).join("");
    }
    ...
    const leaderboardEl = document.getElementById("score-contractor-leaderboard");
    if (leaderboardEl) {
      // Deduplicate contractors
      const contractorMap = new Map();
      roads.forEach(r => {
        if (!contractorMap.has(r.contractorName) || r.contractorPerformance > contractorMap.get(r.contractorName).score) {
          contractorMap.set(r.contractorName, {
            name: r.contractorName,
            score: r.contractorPerformance,
            road: r.name
          });
        }
      });

      const contractors = [...contractorMap.values()].sort((a, b) => b.score - a.score);

      leaderboardEl.innerHTML = contractors.map((c, idx) => {
        const rankClass = idx < 3 ? `rank-${idx + 1}` : "rank-other";
        return `<div class="contractor-leaderboard-item">
          <div class="contractor-rank ${rankClass}">${idx + 1}</div>
          <div class="contractor-info">
            <h4>${this.escapeHTML(c.name)}</h4>
            <p>${this.escapeHTML(c.road)}</p>
          </div>
          <div class="contractor-score">★${c.score}</div>
        </div>`;
      }).join("");
    }

    // Budget health
    const budgetEl = document.getElementById("score-budget-health");
    if (budgetEl) {
      budgetEl.innerHTML = roads.map(road => {
        const pct = Math.min((road.spentBudget / road.sanctionedBudget) * 100, 130);
        const isOverrun = road.spentBudget > road.sanctionedBudget;
        const fillClass = isOverrun ? "overrun" : pct > 90 ? "warning" : "healthy";
        const spentCr = (road.spentBudget / 10000000).toFixed(1);
        const sanctCr = (road.sanctionedBudget / 10000000).toFixed(1);

        return `<div class="budget-health-item">
          <div class="budget-health-header">
            <span>${this.escapeHTML(road.id)}</span>
            <strong>₹${spentCr} / ₹${sanctCr} Cr ${isOverrun ? '⚠️' : '✅'}</strong>
          </div>
          <div class="budget-health-bar">
            <div class="budget-health-fill ${fillClass}" style="width: ${Math.min(pct, 100)}%"></div>
          </div>
        </div>`;
      }).join("");
    }
